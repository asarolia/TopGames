...
export class GameListComponent implements OnInit {
  ...
  constructor(private topGamelistFromService: GamedataService) {
  ...
  ngOnInit():void {
    ...
    // comment out below for service as dependency injetion 
    ...
    // fetch data from service , below method returns hardcoded data from service file
    //this.serviceData = this.topGamelistFromService.getGameData();

    // Subscribe to observable returned via HTTP service
    this.topGamelistFromService.getGameDataAsService().subscribe(
      responseData => {
        this.serviceData = responseData;
        // process response   
     ...
     try {
       if (this.myStorage.length === 0) {
         ...
         for (let index = 0; index < this.serviceData.length; index++) {
           ...
           let { Rank, Name, Genre, Global_Sales, Platform, Publisher, Year } = this.serviceData[index];
